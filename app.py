...
if menu == "1. 데이터 업로드 및 관리":
    ...
    if referral_file:
        ...
        if not df_ref_raw.empty:
            ...
            # A, B, C, D, M, AT, AU, AV (인덱스: 0, 1, 2, 3, 12, 45, 46, 47)
    ...
    if doc:
        with st.spinner("데이터 매칭 및 필터링 적용 중..."):
            try:
                ...
                # [💡 오류 해결 핵심] 적립율 매핑 사전 (제목 행 '전화번호' 등 건너뛰기)
                rate_dict = {}
                for row in rate_values:
                    if len(row) >= 2:
                        biz = str(row[0]).replace('-', '').strip()
                        rate_str = str(row[1]).replace('%','').strip()
                        # 숫자로 변환 가능한 경우만 사전에 추가
                        try:
                            val = float(rate_str)
                            rate_dict[biz] = val/100 if val > 1 else val
                        except ValueError:
                            continue # '전화번호' 같은 텍스트가 나오면 무시하고 다음 행으로
                ...
                for _, ref_row in ref_df.iterrows():
                    ...
                    for _, r_row in matched_rows.iterrows():
                        ...
                        rate = rate_dict.get(rec_biz_clean, 0.03) # 사전에 없으면 기본 3%
